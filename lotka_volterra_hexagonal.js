...
window.onload = function() {
    ...
    const lv_reaction_prey = gpu.createKernel(
        function(prey, predator, dt, param_a, param_b) {
            ...
            const x = this.thread.x;
            const y = this.thread.y;
            const dx = (Math.floor(y % 2) == 0) ? -1 : 0;
            const a = (get(prey, h, w, y - 1, x + dx) +
                       get(prey, h, w, y - 1, x + dx + 1) +
                       get(prey, h, w, y, x - 1) +
                       get(prey, h, w, y, x + 1) +
                       get(prey, h, w, y + 1, x + dx) +
                       get(prey, h, w, y + 1, x + dx + 1)) / 6.0;
    ...
    const lv_reaction_predator = gpu.createKernel(
        function(prey, predator, dt, param_c, param_d) {
            ...
            const dx = (Math.floor(y % 2) == 0) ? -1 : 0;
            const b = (get(predator, h, w, y - 1, x + dx) +
                       get(predator, h, w, y - 1, x + dx + 1) +
                       get(predator, h, w, y, x - 1) +
                       get(predator, h, w, y, x + 1) +
                       get(predator, h, w, y + 1, x + dx) +
                       get(predator, h, w, y + 1, x + dx + 1)) / 6.0;
            return limit(b
                         - param_c * b * dt
